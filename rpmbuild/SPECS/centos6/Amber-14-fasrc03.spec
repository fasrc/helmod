...
%define buildcomments Built with CUDA 6 on aagk80gpu15, using AmberTools15.  
%define requestor Stuart Lopez <lopez01@fas.harvard.edu>
%define requestref RCRT:92408
...
test "%{comp_name}" == "intel" &&  ./configure -cuda -mpi -intelmpi -mkl intel
test "%{comp_name}" == "gcc" && ./configure -cuda -mpi gnu

test "%{comp_name}" == "intel" && sed -i -e 's?^PMEMD_F90=mpiifort?PMEMD_F90=mpiifort -I\$\$INTEL_HOME/mkl/include/fftw ?' src/config.h

make install

make clean
test "%{comp_name}" == "intel" &&  ./configure -mpi -intelmpi -mkl intel
