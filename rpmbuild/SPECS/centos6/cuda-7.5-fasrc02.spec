...
for i in string.gmatch("%{rundependencies}","%%S+") do 
    if mode()=="load" then
        assert(os.execute("test -d /usr/local/cuda-7.5")==0,"Cuda 7.5 is not available from /usr/local/cuda-7.5")
    end
end
...
setenv("CUDA_LIB",                  "/usr/local/cuda-7.5/lib64")
setenv("CUDA_INCLUDE",              "/usr/local/cuda-7.5/include")
prepend_path("PATH",                "/usr/local/cuda-7.5/bin")
prepend_path("CPATH",               "/usr/local/cuda-7.5/include")
prepend_path("FPATH",               "/usr/local/cuda-7.5/include")
prepend_path("LD_LIBRARY_PATH",     "/usr/local/cuda-7.5/lib")
