...
# applicable
#
URL:https://www.haskell.org/ghc/dist/7.6.1/ghc-7.6.1-x86_64-unknown-linux.tar.bz2 
Source: %{name}-%{version}-x86_64-unknown-linux.tar.bz2
...
rm -rf %{name}-%{version}
tar xvf "$FASRCSW_DEV"/rpmbuild/SOURCES/%{name}-%{version}-x86_64-unknown-linux.tar.*
cd %{name}-%{version}
