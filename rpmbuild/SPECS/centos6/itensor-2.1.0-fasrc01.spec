...
#
%define summary_static ITensor—Intelligent Tensor—is a C++ library for implementing tensor network calculations
...
#
%description
ITensor—Intelligent Tensor—is a C++ library for implementing tensor network calculations
...
rm -rf %{name}-%{version}
tar zxf  "$FASRCSW_DEV"/rpmbuild/SOURCES/%{name}-%{version}.tar.gz
mv ITensor-ITensor-c85faf7  %{name}-%{version}
cd %{name}-%{version}
...
PLATFORM=mkl
BLAS_LAPACK_LIBFLAGS=-L/n/sw/intel-cluster-studio-2017/mkl/lib/intel64  -lmkl_intel_lp64 -lmkl_sequential -lmkl_core  -lpthread
BLAS_LAPACK_INCLUDEFLAGS=-I/n/sw/intel-cluster-studio-2017/mkl/include
...
echo THIS_DIR=%{_prefix} > %{buildroot}/%{_prefix}/this_dir.mk 
