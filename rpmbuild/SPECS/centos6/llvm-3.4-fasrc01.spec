...
Source2: compiler-rt-%{version}.src.tar.gz
...
%define requestor Brandon Reagan <reagen@fas.harvard.edu>
%define requestref RCRT:86406
...
rm -rf %{name}-%{version}
rm -rf clang-%{version}
rm -rf compiler-rt-%{version}
tar xvf "$FASRCSW_DEV"/rpmbuild/SOURCES/%{name}-%{version}.src.tar.*
tar xvf "$FASRCSW_DEV"/rpmbuild/SOURCES/clang-%{version}.src.tar.*
tar xvf "$FASRCSW_DEV"/rpmbuild/SOURCES/compiler-rt-%{version}.src.tar.*
mkdir -p %{name}-%{version}/tools
mkdir -p %{name}-%{version}/projects
cp -r clang-%{version} %{name}-%{version}/tools/clang
cp -r compiler-rt-3.4  %{name}-%{version}/projects/compiler-rt
cd %{name}-%{version}
...
mkdir build
cd build
cmake -DBUILD_SHARED_LIBS:BOOL=ON -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=%{_prefix} ..
...
setenv("CLANG_INCLUDE",             "%{_prefix}/lib/clang/3.5.1/include")
setenv("LLVM_LIB",                 "%{_prefix}/lib")
setenv("CLANG_LIB",                 "%{_prefix}/lib/clang/3.5.1/lib")
