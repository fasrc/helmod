...
#
%define summary_static RMBlast (NCBI BLAST for RM) 2.2.28
...
mv %{name}-%{version}-src %{name}-%{version}
cd %{name}-%{version}
...
umask 022
cd "$FASRCSW_DEV"/rpmbuild/BUILD/%{name}-%{version}/c++
...
umask 022
cd "$FASRCSW_DEV"/rpmbuild/BUILD/%{name}-%{version}/c++/GCC447-ReleaseMT64/
echo %{buildroot} | grep -q %{name}-%{version} && rm -rf %{buildroot}
mkdir -p %{buildroot}/%{_prefix}
#make install DESTDIR=%{buildroot}
cp -r {bin,build,inc,lib,status} %{buildroot}%{_prefix}
...
setenv("RMBLAST_HOME",                "%{_prefix}")
setenv("RMBLASTBLAST_INCLUDE",        "%{_prefix}/inc")
setenv("RMBLASTBLAST_LIB",            "%{_prefix}/lib")
