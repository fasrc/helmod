...
#
%define summary_static NCO version 4.2.5
...
rm -rf nco-%{version}
tar xvf "$FASRCSW_DEV"/rpmbuild/SOURCES/nco-%{version}.tar.*
cd nco-%{version}
...
umask 022
cd "$FASRCSW_DEV"/rpmbuild/BUILD/nco-%{version}
...
umask 022
cd "$FASRCSW_DEV"/rpmbuild/BUILD/nco-%{version}
echo %{buildroot} | grep -q nco-%{version} && rm -rf %{buildroot}
