...
#
%define summary_static Python version 2.7.9
...
%define buildcomments This python is being built as a Comp module so that it is available for Intel-specific code. 
...
# For intel, the byte-compiling fails on some test files, so ignore
sed -i -e 's?\t\(PYTHONPATH=$(DESTDIR)$(LIBDEST) $(RUNSHARED)\)?\t-\1?' Makefile
sed -i -e 's?\t\(PYTHONPATH=$(DESTDIR)$(LIBDEST)  $(RUNSHARED)\)?\t-\1?' Makefile
...
setenv("PYTHON_HOME",              "%{_prefix}")
setenv("PYTHON_INCLUDE",           "%{_prefix}/include")
setenv("PYTHON_LIB",               "%{_prefix}/lib")
