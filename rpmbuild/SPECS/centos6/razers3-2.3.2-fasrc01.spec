...
#
%define summary_static RazerS 3 is a tool for mapping millions of short genomic reads onto a reference genome
...
%define builddependencies cmake/3.5.2-fasrc01 seqan/2.3.2-fasrc01 boost/1.55.0-fasrc01 zlib/1.2.8-fasrc02
%define rundependencies %{builddependencies}
%define buildcomments This installs razers3 from the seqan source checkout
...
rm -rf seqan-seqan-v%{version}
tar xvf "$FASRCSW_DEV"/rpmbuild/SOURCES/seqan-v%{version}.tar.*
cd seqan-seqan-v%{version}
...
umask 022
cd "$FASRCSW_DEV"/rpmbuild/BUILD/seqan-seqan-v%{version}
...
cmake -DCMAKE_INSTALL_PREFIX=%{_prefix} -DCMAKE_INCLUDE_PATH:STRING="$ZLIB_INCLUDE;$BOOST_INCLUDE;$ICU_INCLUDE" -DCMAKE_LIBRARY_PATH:STRING="$ZLIB_LIB;$BOOST_LIB" -DSEQAN_BUILD_SYSTEM=APP:razers3 ..
...
#           -e "s?/usr/lib64/libicui18n.so?$ICU_LIB/libicui18n.so?" apps/bs_tools/CMakeFiles/casbar.dir/build.make
# 
#sed -i    -e "s?/usr/lib64/lib64/libboost_wave-mt.so.5?$BOOST_LIB/libboost_wave.so?" \
#           -e "s?/usr/lib64/lib64/libboost_signals-mt.so.5?$BOOST_LIB/libboost_signals.so?" \
#           -e "s?/usr/lib64/lib64/libboost_program_options-mt.so.5?$BOOST_LIB/libboost_program_options.so?" \
...
#           -e "s?/usr/lib64/lib64/libboost_date_time-mt.so.5?$BOOST_LIB/libboost_date_time.so?" apps/bs_tools/CMakeFiles/casbar.dir/link.txt
...
umask 022
cd "$FASRCSW_DEV"/rpmbuild/BUILD/seqan-seqan-v%{version}/build
echo %{buildroot} | grep -q seqan-%{version} && rm -rf %{buildroot}
