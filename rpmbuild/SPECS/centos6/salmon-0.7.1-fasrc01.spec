...
%define builddependencies tbb/4.4-fasrc02 boost/1.55.0-fasrc03 cmake/2.8.3-fasrc01 xz/5.2.2-fasrc01
%define rundependencies  tbb/4.4-fasrc02 boost/1.55.0-fasrc03 xz/5.2.2-fasrc01 
...
sed -i -e '/enable_testing/d' CMakeLists.txt
sed -i -e '/TARGET_FILE_NAME:unitTests/d' \
       -e '/add_test( NAME unit_tests/d' \
       -e '/add_test( NAME salmon_read_test_fmd/d' \
       -e '/add_test( NAME salmon_read_test_quasi/d' src/CMakeLists.txt


mkdir build; cd build

export CXX="g++ -I$TBB_INCLUDE -L$TBB_LIB"
cmake -DBOOST_ROOT=$BOOST_HOME -DTBB_INSTALL_DIR=$TBB_HOME -DCMAKE_INSTALL_PREFIX=%{_prefix} ..
...
cp src/{salmon,libsalmon_core.a} %{buildroot}/%{_prefix}
