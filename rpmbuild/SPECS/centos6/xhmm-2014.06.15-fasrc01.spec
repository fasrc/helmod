...
#
%define summary_static eXome-Hidden Markov Model
...
# applicable
#
URL: https://atgu.mgh.harvard.edu/xhmm/index.shtml
#$ wget -O xhmm-2014.06.15.zip https://bitbucket.org/statgen/xhmm/get/master.zip
#	unzips as statgen-xhmm-9f92331916d5
#	git repo last updated 2014-06-15, so I'll use that date
...
%define builddependencies R/3.1.0-fasrc01
...
#this is not a prerequisite, but just needed to build and use the extra R stuff
...
cp build/execs/xhmm %{buildroot}/%{_prefix}/bin/
...
mkdir %{buildroot}/%{_prefix}/R

#the R package needs 'gplots' and 'plotrix'
...
#xhmmScripts_1.1.tar.gz for the 2014.06.15 version
echo 'install.packages(list.files(path=".", pattern="xhmmScripts_.+\\.tar\\.gz"), lib="%{_prefix}/R", repos=NULL)' | R --vanilla

#the above will fail silently; check that it built something
stat %{_prefix}/R/xhmmScripts*
...
-- R is not a tight dependency -- it is only needed if using the associated R 
-- package.
...
prepend_path("R_LIBS_USER",         "%{_prefix}/R")
EOF
