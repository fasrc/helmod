...
#
URL: ftp://ftp.ncbi.nlm.nih.gov/blast/executables/blast+/LATEST/ncbi-blast-2.8.1+-x64-linux.tar.gz
Source: %{name}-%{version}-x64-linux.tar.gz
...
%define buildcomments Built from the pre-compiled binary
%define requestor Patrick Kimes
%define requestref RCRT:131853
...
# %%setup
cd %{_topdir}/BUILD
tar xvf %{_topdir}/SOURCES/%{name}-%{version}-x64-linux.tar.*
