...
%define buildcomments Activation module for CUDA libraries.  Doesn't actually install them, but updates PATH, LD_LIBRARY_PATH, etc. MODULEPATHs updated to accommodate FASRCSW_CUDAS
...
for i in string.gmatch("%{rundependencies}","%%S+") do 
    if mode()=="load" then
        assert(os.execute("test -d /usr/local/cuda-6.5")==0,"Cuda 6.5 is not available from /usr/local/cuda-6.5")
    end
end
...
setenv("CUDA_LIB",                  "/usr/local/cuda-6.5/lib64")
setenv("CUDA_INCLUDE",              "/usr/local/cuda-6.5/include")
prepend_path("PATH",                "/usr/local/cuda-6.5/bin")
prepend_path("CPATH",               "/usr/local/cuda-6.5/include")
prepend_path("FPATH",               "/usr/local/cuda-6.5/include")
prepend_path("LD_LIBRARY_PATH",     "/usr/local/cuda-6.5/lib")
prepend_path("LIBRARY_PATH",        "/usr/local/cuda-6.5/lib")
prepend_path("LD_LIBRARY_PATH",     "/usr/local/cuda-6.5/lib64")
prepend_path("LIBRARY_PATH",        "/usr/local/cuda-6.5/lib64")
