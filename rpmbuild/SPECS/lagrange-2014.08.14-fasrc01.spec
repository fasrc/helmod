#
# export NAME=lagrange
# export VERSION=2014.08.14
# export RELEASE=fasrc01
# export TYPE=Core
#
# There is a setup.py, but all it is a single package of py files, so this spec 
# just copies the repo contents and sets PYTHONPATH to point directly to it.
#
...
# applicable
#
URL: http://code.google.com/p/lagrange/
...
git clone https://github.com/rhr/lagrange-python %{name}-%{version}
cd %{name}-%{version}
git checkout 5f346c5942e880dd868e0ea285f4fb349d9e2906  #the latest as of creating this spec file
...
#forklift the whole thing (except setup.py, since this dir is going in PYTHONPATH)
rm setup.py
...
if mode()=="load" then
	if not isloaded("Anaconda") then
		load("Anaconda/1.9.2-fasrc01")
	end
end
