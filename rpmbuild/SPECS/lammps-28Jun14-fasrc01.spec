...
sed -i -e 's?^FFT_INC .*?FFT_INC = -DFFT_NONE?' MAKE/Makefile.openmpi
sed -i -e 's?^FFT_LIB .*?FFT_LIB =?' MAKE/Makefile.openmpi
sed -i -e 's?^MPI_INC .*?MPI_INC =?' MAKE/Makefile.openmpi
sed -i -e 's?^MPI_PATH .*?MPI_PATH = -L\${MPI_LIB}?' MAKE/Makefile.openmpi

test %{mpi_name} == 'mvapich2' && sed -i -e 's?^MPI_LIB .*?MPI_LIB = -lmpich -lpthread?' MAKE/Makefile.openmpi ||  sed -i -e 's?^MPI_LIB .*?MPI_LIB = -lmpi -lpthread?' MAKE/Makefile.openmpi
