...
#
%define summary_static MrBayes is a program for Bayesian inference and model choice across a wide range of phylogenetic and evolutionary models.
...
#
#http://http://mrbayes.sourceforge.net/download.php
...
cd %{_topdir}/BUILD/%{name}_%{version}/src
autoconf
./configure --prefix=%{_prefix} --enable-mpi=yes --enable-threads=yes --with-beagle=$BEAGLE_PATH
make
...
install -m 755 mb %{buildroot}/%{_prefix}/bin
...
---- prerequisite apps (uncomment and tweak if necessary)
if mode()=="load" then
	if not isloaded("beagle") then
		load("beagle/2.1.trunk-fasrc01")
	end
end
