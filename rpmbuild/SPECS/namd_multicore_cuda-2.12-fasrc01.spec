...
#
%define summary_static NAMD is a parallel molecular dynamics code designed for high-performance simulation of large biomolecular systems
...
%define builddependencies cuda/7.5-fasrc02 tcl/8.6.4-fasrc01 charm_multicore/6.7.1-fasrc01 fftw/3.3.4-fasrc09
...
mv NAMD_%{version}_Source %{name}-%{version}
cd %{name}-%{version}
...
cat > ./arch/Linux-x86_64.cuda  <<EOF
CUDADIR=\${CUDA_HOME}
...
cat > Make.charm <<EOF
CHARMBASE = ${CHARM_HOME}
EOF

./config Linux-x86_64-g++  --with-cuda --with-fftw3 --with-tcl --charm-arch multicore-linux64
cd Linux-x86_64-g++
sed -i 's/\/\$(CHARMARCH)//' Make.config
make -j4
...
for i in inc  Make.config  Make.depends  Makefile  obj  plugins   sb  src
do 
cp -r -L $i %{buildroot}/%{_prefix}/share 
done 
for i in charmrun  flipbinpdb  flipdcd namd2 psfgen  sortreplicas
do 
cp -r -L $i %{buildroot}/%{_prefix}/bin 
done 
