...
#
Packager: Harvard FAS Research Computing -- John Morrissey <jmorrissey@fas.harvard.edu>
...
#
%define summary_static An open source computer vision and machine learning software library
...
#
# https://downloads.sourceforge.net/project/opencvlibrary/opencv-unix/2.4.8/opencv-2.4.8.zip
Source: %{name}-%{version}.zip
Patch0: opencv-cmake-2.6-compat.patch
...
recognize faces, identify objects, classify human actions in videos, track
camera movements, track moving objects, extract 3D models of objects,
...
an image database, remove red eyes from images taken using flash, follow eye
movements, recognize scenery and establish markers to overlay it with
...
cd %{_topdir}/BUILD
rm -rf %{name}-%{version}
unzip -o %{_topdir}/SOURCES/%{name}-%{version}.zip
cd %{name}-%{version}
%patch0 -p1
...
mkdir release
cd release
cmake -D CMAKE_BUILD_TYPE=RELEASE -D CMAKE_INSTALL_PREFIX=%{_prefix} ..
make
