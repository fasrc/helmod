...
#
%define summary_static PETSc version 3.3-p7
...
# applicable
#
URL: http://ftp.mcs.anl.gov/pub/petsc/release-snapshots/petsc-3.4.5.tar.gz
Source: %{name}-%{version}-p7.tar.gz
...
%define builddependencies python/2.7.6-fasrc01 metis/5.1.0-fasrc01 fftw/3.3.4-fasrc06 netcdf/4.1.3-fasrc02 boost/1.54.0-fasrc02 
...
rm -rf %{name}-%{version}-p7
tar xvf "$FASRCSW_DEV"/rpmbuild/SOURCES/%{name}-%{version}-p7.tar.*
cd %{name}-%{version}-p7
...
umask 022
cd "$FASRCSW_DEV"/rpmbuild/BUILD/%{name}-%{version}-p7
...
./configure --prefix=%{_prefix} --with-mpi-dir=$MPI_HOME --with-metis-dir=$METIS_HOME --with-mpi4py --with-numpy --with-fftw-dir=$FFTW_HOME --with-boost-dir=$BOOST_HOME --with-netcdf-dir=$NETCDF_HOME --with-hdf5-dir=$HDF5_HOME --with-shared-libraries
