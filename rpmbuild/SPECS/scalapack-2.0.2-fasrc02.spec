...
#
%define summary_static ScaLAPACK (Scalable LAPACK) is a library of high-performance linear algebra routines for parallel distributed memory machines.
...
%define buildcomments Built with -fPIC using Makefile instead of cmake
%define requestor Norman Yao <norman.yao@gmail.com>
%define requestref RCRT:108225
...
sed -e 's/\(^FCFLAGS.*\)/\1 -fPIC/' -e 's/\(^CCFLAGS.*\)/\1 -fPIC/' < SLmake.inc.example > SLmake.inc
...
cp libscalapack.a %{buildroot}/%{_prefix}/lib 
...
setenv("SCALAPACK_HOME",            "%{_prefix}")
setenv("SCALAPACK_LIB",            "%{_prefix}/lib")
