...
#
%define summary_static tbl2asn is a program that automates the submission of sequence records to GenBank.
...
%define buildcomments Just a binary.  Needed by Cheryl Andam when the one bundled with prokka failed.
...
gzip -dc "$FASRCSW_DEV"/rpmbuild/SOURCES/linux64.%{name}.gz > %{name}
chmod a+rx %{name}
