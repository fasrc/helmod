...
test -z "$FASRCSW_PROD" && export FASRCSW_PROD=/n/sw/helmod-rocky9
...
#the build host
export FASRCSW_BUILD_HOST_01=holy7c26501
if [ "$(hostname -s)" != "$FASRCSW_BUILD_HOST_01" ] && [ "$(hostname -s)" != "$FASRCSW_BUILD_HOST_02" ]; then 
	echo "WARNING: the current host is not the canonical build host for Rocky 9, use builds01." >&2
fi
